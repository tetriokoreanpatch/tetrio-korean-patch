...
with open("../hun.fnt", "r") as r:
    for line in r.readlines():
        ...
        xadvperchar[int(getval(line, "char id"))] = int(getval(line,"xadvance"))
        xoffperchar[int(getval(line, "char id"))] = int(getval(line,"xoffset"))
        yoffperchar[int(getval(line, "char id"))] = int(getval(line,"yoffset"))
        if getval(line, "width") == "0":
            extrachar.append(int(getval(line, "char id")))
...
class FntConfig:
    ...
    @format_str
    def __str__(self):
        self.info["padding"] = (4, 4, 4, 4)
        self.info["spacing"] = (-8, -8)
...
class CharDef:
    ...
    def ini_with_texture_size(self, size):
        ...
        if self.param["id"] not in xadvperchar:
            print("not in adv")
            xadvperchar[self.param["id"]] = 45
        if self.param["id"] not in xoffperchar:
            print("not in xoff")
            xoffperchar[self.param["id"]] = 0
        if self.param["id"] not in yoffperchar:
            print("not in yoff")
            yoffperchar[self.param["id"]] = -4
        
        self.param["width"], self.param["height"] = size[0] + padding[1] + padding[3], size[1] + padding[0] + padding[2]
        
        self.param["xadvance"] = xadvperchar[self.param["id"]]
        self.param["xoffset"] = xoffperchar[self.param["id"]]
        self.param["yoffset"] = yoffperchar[self.param["id"]]
...
class CharSet:
    ...
    def __str__(self):
        ret = 'chars count=' + str(len(self.chars) + len(extrachar)) + '\n'
    ...
    def gen_fnt(self):
        ...
        try:
            with open(fnt_file_name, 'w', encoding='utf8') as fnt:
                ...
                for extra in extrachar:
                    fnt.write("char id=" + str(extra) + " x=0 y=0 width=0 height=0 xoffset=" + str(xoffperchar[extra]) + " yoffset=" + str(yoffperchar[extra]) + " xadvance=" + str(xadvperchar[extra]) + " page=0 chnl=0\n ")
                fnt.write("kernings count=0")
            fnt.close()
        except IOError:
